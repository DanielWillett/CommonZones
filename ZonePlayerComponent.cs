...
internal class ZonePlayerComponent : MonoBehaviour
{
    ...
    private static ushort _editId = 0;
    private static ushort _locId = 0;
    ...
    private static EffectAsset _airdrop = null!;
    private static EffectAsset _center  = null!;
    private static EffectAsset _corner  = null!;
    private static EffectAsset _line    = null!;
    internal static void UIInit()
    {
        _editId     =  Assets.find(CommonZones.I.Configuration.Instance.ZoneEditHUDID)           is EffectAsset ea1 ? ea1.id : (ushort)0;
        _locId      =  Assets.find(CommonZones.I.Configuration.Instance.CurrentZoneEffectID)     is EffectAsset ea2 ? ea2.id : (ushort)0;
        _airdrop    = (Assets.find(EAssetType.EFFECT, 120) as EffectAsset)!;
        _center     = (Assets.find(EAssetType.EFFECT, 113) as EffectAsset)!;
        _corner     = (Assets.find(EAssetType.EFFECT, 115) as EffectAsset)!;
        _line       = (Assets.find(EAssetType.EFFECT, 117) as EffectAsset)!;
    }
        ...
        if (time - _lastCheck >= CommonZones.I.Configuration.Instance.CheckTimeSeconds)
        {
                ...
                else
                {
                    for (int j = _zones.Count - 1; j >= 0; --j)
                    {
                        if (_zones[j].Equals(zone.Name, StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            EffectManager.askEffectClearByID(_locId, player.channel.owner.transportConnection);
                            break;
                        }
                    }
                }
            ...
            if (enterQueue.Count > 0)
            {
                for (int i = 0; i < enterQueue.Count; ++i)
                {
                    ...
                    if (_locId != 0 && i == 0)
                    {
                        EffectManager.sendUIEffect(_locId, short.MaxValue, player.channel.owner.transportConnection, true, z.Name);
...
#pragma warning restore IDE0051
    ...
    internal void CreateCommand(string[] args)
    {
        ...
        EffectManager.sendUIEffect(_editId, EDIT_KEY, tc, true);
    ...
    private void RefreshPreview()
    {
        ...
        EffectManager.askEffectClearByID(_line.id, channel);
            ...
            case EZoneType.CIRCLE:
                if (!float.IsNaN(_currentBuilder.ZoneData.Radius))
                {
                    ...
                    for (int i = 0; i < points.Length; i++)
                    {
                        ...
                        pos.y = Util.GetHeight(pos, _currentBuilder.MinHeight);
                        Util.TriggerEffectReliable(_line, channel, pos); // yellow paintball splatter
                        Util.TriggerEffectReliable(_airdrop, channel, pos); // airdrop
                    }
                }
                break;
    ...
    internal void EditCommand(string[] args)
    {
        ...
        else if (operation.Equals("finalize", StringComparison.OrdinalIgnoreCase) || operation.Equals("complete", StringComparison.OrdinalIgnoreCase))
        {
            try
            {
                ...
                EffectManager.askEffectClearByID(_editId, player.channel.owner.transportConnection);
                _currentBuilderIsExisting = false;
                RefreshPreview();
            }
            catch (Exception ex)
